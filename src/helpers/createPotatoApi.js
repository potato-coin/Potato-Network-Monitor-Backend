...
const createPotatoApi = ({ host = NODE.HOST, port = NODE.PORT, isVariable = true, onlyRequest = false } = {}) => {
  ...
  let potatoRpc = new JsonRpc(`${host}:${port}`, { fetch });
  ...
  const changeNode = () => {
    ...
    potatoRpc = new JsonRpc(`${currentNode.HOST}:${currentNode.PORT}`, { fetch });
  ...
  const get_info = async () => potatoRpc.get_info();
  const get_block = async (blockNumOrId) => potatoRpc.get_block(blockNumOrId);
  const get_account = async (accountName) => potatoRpc.get_account(accountName);
  const get_producers = async (json = true, lowerBound = null, limit = null) => potatoRpc.get_producers(json, lowerBound, limit);
  const get_producer_schedule = async () => potatoRpc.get_producer_schedule();
  const get_currency_balance = async (code, account, symbol = null) => potatoRpc.get_currency_balance(code, account, symbol);
  const get_currency_stats = async (code, symbol) => potatoRpc.get_currency_stats(code, symbol);
  const get_table_rows = async (args) => potatoRpc.get_table_rows(args);
  const get_table_by_scope = async (args) => potatoRpc.get_table_by_scope(args);
